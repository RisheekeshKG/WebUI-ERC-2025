...
ros.on('connection',   () => { updateStatus(webStatusEl,'Connected ✅');      updateStatus(twistStatusEl,'Idle ⚪','#ccc'); logDebug('✅ Connected to rosbridge.'); });
ros.on('error',        e  => { updateStatus(webStatusEl,'Error ❌','#f55');    updateStatus(twistStatusEl,'Error ❌','#f55'); logDebug(`❌ ROS error: ${e}`);   });
ros.on('close',        () => { updateStatus(webStatusEl,'Disconnected ⚠️','#fa0'); updateStatus(twistStatusEl,'Disconnected ⚠️','#fa0'); logDebug('⚠️ Disconnected.'); });

// === Velocity Multipliers (sliders) -------------------------------------
let linearMul  = 0.5;
let angularMul = 0.5;
...
function pollGamepad() {
  ...
  if (gp) {
    ...
    const dz = 0.1;              // dead‑zone
    const lin = Math.abs(y) > dz ? -y * linearMul  : 0;
    const ang = Math.abs(x) > dz ? -x * angularMul : 0;
...
if (ctx) {
  new ROSLIB.Topic({
    ...
    const cx = canvas.width/2, cy = canvas.height/2, scale = 40;
...
// === Camera Stream(s) ----------------------------------------------------
const camTopics = { cam1:'/camera/image_raw' };   // add more if needed
Object.entries(camTopics).forEach(([id,topic])=>{
  const img = document.getElementById(id);
  if (!img) return;
  img.src = `http://localhost:8080/stream?topic=${topic}&type=ros_compressed`;
  img.onload  = ()=>{ img.style.opacity=1; logDebug(`📷 ${id} stream OK.`); };
  img.onerror = ()=>{ img.style.opacity=0.2; img.alt='No stream'; logDebug(`🚫 ${id} stream fail.`);};
});
